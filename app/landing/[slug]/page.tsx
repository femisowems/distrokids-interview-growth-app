...
import { landingSections } from '@/lib/mock';
import { landingPageMetadata, schemaForReleasePage } from '@/lib/landing-page';
import { buildMetaTags } from '@/lib/seo';

const releasesBySlug: Record<string, { title: string; artist: string; theme: string }> = {
  afterimage: { title: 'Afterimage', artist: 'Nova Rue', theme: 'Midnight Neon' }
};

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params;
  const release = releasesBySlug[slug];
  if (!release) return {};
  return buildMetaTags(landingPageMetadata(release.title, release.artist).title, landingPageMetadata(release.title, release.artist).description, `/landing/${slug}`);
}
...
export default async function LandingPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const schema = schemaForReleasePage(release.title, release.artist, `/landing/${slug}`);
